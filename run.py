...
from amanuensis.api import run_for_development
...
def run_with_fake_auth():
    ...
    with patch(
        "amanuensis.auth.FederatedUser.roles",
        new_callable=PropertyMock,
        return_value=roles,
    ), patch(
        "amanuensis.auth.FederatedUser.logged_in",
        new_callable=PropertyMock,
        return_value=lambda: True,
    ), patch(
        "amanuensis.auth.FederatedUser.get_project_ids",
        new_callable=PropertyMock,
        return_value=get_project_ids,
    ), patch(
        "amanuensis.auth.verify_hmac", new=set_user
    ):
...
def run_with_fake_download():
    with patch("amanuensis.download.get_nodes", fake_get_nodes):
        with patch.multiple(
            "amanuensis.download",
...
if __name__ == "__main__":
    debug = bool(os.environ.get("AMANUENSIS_DEBUG", True))
