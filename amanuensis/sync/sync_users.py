...
def arborist_role_for_permission(permission):
    """
    For the programs/projects in the existing amanuensis access control model, in order to
...
class UserYAML(object):
    ...
    def from_file(cls, filepath, encrypted=True, key=None, logger=None):
        """
        Add access by "auth_id" to "self.projects" to update the amanuensis DB.
...
class UserSyncer(object):
                ...
                for row in csv:
                    ...
                    if len(phsid) > 1 and parse_consent_code:
                        ...
                        if (
                            ...
                            self.logger.info(
                                "found study with consent c999 and amanuensis "
    ...
    def _add_dbgap_project_for_user(
        ...
        """
        Helper function for csv parsing that adds a given dbgap project to amanuensis/Arborist
        and then updates the dictionary containing all user's project access
        """
        if dbgap_project not in self._projects:
            self.logger.debug(
                "creating Project in amanuensis for dbGaP study: {}".format(dbgap_project)
            )
    ...
    def sync_to_db_and_storage_backend(self, user_project, user_info, sess):
        ...
        auth_provider_list = [
            ...
            self._get_or_create(sess, AuthorizationProvider, name="amanuensis"),
        ]
    ...
    def _process_dbgap_files(self, dbgap_config, sess):
        ...
        Args:
            dbgap_config : a dictionary containing information about a single
                           dbgap sftp server (from amanuensis config)
    ...
    def _merge_multiple_dbgap_sftp(self, dbgap_servers, sess):
        ...
        Args:
            dbgap_servers : a list of dictionaries each containging config on
                           dbgap sftp server (comes from amanuensis config)
            sess: database session
    ...
    def _sync(self, sess):
        ...
        # come from the first dbgap instance in the amanuensis config file
    ...
    def _update_authz_in_arborist(self, session, user_projects, user_yaml=None):
        ...
        try:
            ...
            # removing access, the case in the amanuensis db is used. For combining access, it is
                        ...
                        if policy_id not in self._created_policies:
                            try:
                                self.arborist_client.update_policy(
                                    policy_id,
                                    {
                                        "description": "policy created by amanuensis sync",
                                        "role_ids": [permission],
                                        "resource_paths": [path],
                                    },
        ...
        for client_name, client_details in user_yaml.clients.items():
            ...
            if not client:
                self.logger.warning(
                    "client to update (`{}`) does not exist in amanuensis: skipping".format(
                        client_name
                    )
                )
                continue
