...
from amanuensis.models import GoogleServiceAccountKey
...
class GoogleCredentialsList(Resource):
        ...
        with GoogleCloudManager() as g_cloud_manager:
            ...
            for i, key in enumerate(keys):
                ...
                # happen! Delete the key from Google
                    ...
                    keys.pop(i)
                    logger.warning(
                        "No GoogleServiceAccountKey entry was found in the amanuensis database for service account name {} for key_id {}, which exists in Google. It will now be deleted from Google.".format(
                            username, key_id
                        )
                    )
    ...
    def handle_user_service_account_creds(self, key, service_account):
        ...
        See amanuensis-create scripting functions for expiration logic.

        The reason for this difference is due to the fact that amanuensis itself
