...
class IndexedFile(object):
    ...
    def check_authz(self, action):
        ...
        logger.debug(
            f"authz check can user {action} on {self.index_document['authz']} for amanuensis?"
        )
...
class GoogleStorageIndexedFileLocation(IndexedFileLocation):
    ...
    def _generate_anonymous_google_storage_signed_url(
        ...
        # we will use the main amanuensis SA service account to sign anonymous requests
