...
from amanuensis.auth import login_required, require_auth_header, current_token, get_jwt
from amanuensis.blueprints.data.indexd import (
    BlankIndex,
    IndexedFile,
...
def delete_data_file(file_id):
    ...
    if authz:
        logger.debug(
            "Trying to ask arborist if user can delete in amanuensis for {}".format(authz)
        )
        has_correct_authz = flask.current_app.arborist.auth_request(
            jwt=get_jwt(), service="amanuensis", methods="delete", resources=authz
        )
