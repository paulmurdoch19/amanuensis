...
amanuensis, which will be linked to in the OAuth2 consent screens.
...
- If there is a PRIVACY_POLICY_URL environment variable (which should come from the
  manifest, exposed here through the amanuensis deploy file---see manifest and
  `cloud-automation/kube/services/amanuensis/`), then redirect to that URL.
- If the file `amanuensis/static/privacy_policy.md` exists, then return that either as
  markdown or HTML depending on the accept mimetype. This can also be handled via cloud
  automation with a volume mount adding this file, or if you are deploying amanuensis
...
def privacy_policy():
    ...
    if not cache.has("privacy-policy-md"):
        file_contents = pkgutil.get_data("amanuensis", "static/privacy_policy.md").decode(
            "utf-8"
        )
