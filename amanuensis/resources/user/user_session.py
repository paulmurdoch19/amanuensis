...
from amanuensis.jwt.keys import default_public_key
from amanuensis.jwt.token import generate_signed_access_token, generate_signed_session_token
from amanuensis.jwt.validate import validate_jwt
from amanuensis.jwt.validate import JWTError
from amanuensis.resources.storage.cdis_jwt import create_session_token
from amanuensis.user import get_current_user
from amanuensis.resources.google.utils import get_linked_google_account_email
...
class UserSession(SessionMixin):
    def __init__(self, session_token):
        ...
        if session_token:
            try:
                jwt_info = validate_jwt(session_token, aud={"amanuensis"})
            except JWTError:
...
def _get_user_id_from_access_token(access_token):
    ...
    if token_user_id:
        try:
            ...
            # if we can't cast to an int, that's an issue. amanuensis should
