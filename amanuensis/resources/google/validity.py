...
class GoogleProjectValidity(ValidityInfo):
    ...
    def check_validity(self, early_return=True, db=None):
        ...
        if not has_access:
            logger.warning(
                "INVALID amanuensis's Monitoring service account does "
        ...
        logger.debug(
            "Determining if valid users exist in amanuensis.".format(self.google_project_id)
        )
        # if we have valid members, we can check if they exist in amanuensis
        ...
        if user_members is not None:
            try:
                ...
                logger.warning(
                    "INVALID user(s) do not exist in amanuensis and thus, "
...
class GoogleServiceAccountValidity(ValidityInfo):
    ...
    def check_validity(
        ...
        # it is invalid, however, if amanuensis has access to the SA's
