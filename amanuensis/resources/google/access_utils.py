...
from amanuensis.errors import NotFound, NotSupported, UserError
...
def get_google_project_number(google_project_id, google_cloud_manager):
    ...
    This will only be successful if amanuensis can access info about the given google project
...
def get_user_by_email(user_email, db=None):
    """
    Return user from amanuensis DB

    Args:
        user_id (str): user's amanuensis email id

    Returns:
        bool: user in amanuensis DB with user_email
    """
...
def user_has_access_to_project(user, project_id, db=None):
    ...
    Args:
        user (amanuensis.model.User): user to check access
...
def do_all_users_have_access_to_project(users, project_id, db=None):
    session = get_db_session(db)
    # users will be list of amanuensis.model.User's
...
def patch_user_service_account(
    ...
    - Add and remove project access and bucket groups to/from amanuensis db
    ...
    if not service_account:
        raise amanuensis.errors.NotFound(
...
def get_project_ids_from_project_auth_ids(session, auth_ids):
    ...
    for project_auth_id in auth_ids:
        ...
        if not project:
            raise amanuensis.errors.NotFound(
                "There is no {} in amanuensis db".format(project_auth_id)
            )
...
def get_google_access_groups_for_service_account(service_account):
    """
    Return list of amanuensis.models.GoogleBucketAccessGroup objects that the
    given service account should have access to based on it's access
    privileges.

    Args:
        service_account (amanuensis.models.UserServiceAccount): service account
            object

    Returns:
        List[amanuensis.models.GoogleBucketAccessGroup]: list of google bucket
