...
from amanuensis.auth import current_token
from amanuensis.config import config
from amanuensis.errors import NotSupported, InternalError, UserError
...
from amanuensis.resources.google import STORAGE_ACCESS_PROVIDER_NAME
from amanuensis.errors import NotSupported, NotFound
...
def give_service_account_billing_access_if_necessary(
    ...
    if r_pays_project:
        ...
        try:
            ...
            # NOTE: this may fail if our amanuensis SA doesn't have the right permissions
...
def get_service_account(client_id, user_id):
    """
    Return the service account (from amanuensis db) for given client.
    ...
    Returns:
        amanuensis.models.GoogleServiceAccount: Client's service account
    """
    service_account = (
...
def get_or_create_service_account(client_id, user_id, username, proxy_group_id):
    ...
    Returns:
        amanuensis.models.GoogleServiceAccount: New service account
    """
    if proxy_group_id:
        if client_id:
...
def get_users_from_google_members(members, db=None):
    ...
    Return:
        List[amanuensis.models.User]: Users from our db for members on Google project
...
def get_user_from_google_member(member, db=None):
    ...
    Return:
        amanuensis.models.User: User from our db for member on Google project
    """
    session = get_db_session(db)
