...
from amanuensis.errors import NotFound, UserError, UnavailableError
...
from amanuensis.resources import group as gp, project as pj, user as us, userdatamodel as udm
...
def delete_google_service_accounts_and_keys(current_session, gcm, gpg_email):
    """
    Delete from both Google and amanuensis all Google service accounts and
    service account keys associated with one Google proxy group.
    """
    logger.debug("Deleting all associated service accounts...")

    # Referring to cirrus for list of SAs. You _could_ refer to amanuensis db instead.
    ...
    for sae in service_account_emails:
        ...
        logger.debug(
            "Attempting to clear service account records from amanuensis database..."
        )
        sa = (
        ...
        if sa:
            ...
            logger.info(
                "Records for service account {} successfully cleared from amanuensis database.".format(
                    sae
                )
            )
        else:
            logger.info(
                "Records for service account {} NOT FOUND in amanuensis database. "
...
def delete_google_proxy_group(
    ...
    """
    Delete a Google proxy group from both Google and amanuensis.

    google_proxy_group_from_fence_db is the GPG row in amanuensis. If there is ever the case where
    the GPG exists in Google but is not in the amanuensis db, google_proxy_group_from_fence_db will be None
    but there will still be a GPG to delete from Google.

    user is the User row in amanuensis.
    """
    ...
    if google_proxy_group_from_fence_db:
        # (else it was google_proxy_group_from_google and there is nothing to delete in amanuensis db.)
        logger.debug("Attempting to clear proxy group records from amanuensis database...")
        logger.debug(
        ...
        current_session.commit()
        logger.info(
            "Records for Google proxy group {} successfully cleared from amanuensis "
...
def delete_user(current_session, username):
    ...
    with GoogleCloudManager() as gcm:
        ...
        if google_proxy_group_from_fence_db:
            ...
            logger.debug("Found Google proxy group in amanuensis db: {}".format(gpg_email))
        else:
