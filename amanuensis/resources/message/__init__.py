...
from amanuensis.resources.userdatamodel import get_all_messages, get_messages_by_request, send_message, get_user
...
def send_message(logged_user_id, request_id, subject, body):
    with flask.current_app.db.session as session:    
        ...
        req = request.get_by_id(session, logged_user_id, request_id)
        ...
        if hubspot_response:
            for member in hubspot_response["results"]:
                ...
                emails.append(email)
                ec_user = udm.get_user(session, email)
